...
def draw_label(prediction, save_root, name):
    '''
    Draw the mask probability for each img and save them to the save path
   :param prediction: [2, h, w] numpy
   :param save_path: string including img name
   :return: None
   '''
    if not os.path.exists(os.path.join(save_root, 'label')):
        os.makedirs(os.path.join(save_root, 'label'))
    prediction = prediction
    prediction = construct_color_img(prediction)
    cv2.imwrite(os.path.join(save_root, 'label', name.split('.')[0]) + '.png', prediction)
